...
from gym_pybullet_drones.envs.BaseEnv import BaseEnv
from gym_pybullet_drones.utils.enums import DroneModel, Physics, ActionType, ObservationType, ImageType
from gym_pybullet_drones.control.DSLPIDControl import DSLPIDControl

class BaseRLEnv(BaseEnv):
    ...
    def _addObstacles(self):
        ...
        Only if the observation is of type RGB, 4 landmarks are addself.CLIENTed.
    ...
    def _observationSpace(self):
        ...
        elif self.OBS_TYPE == ObservationType.KIND:
            ############################################################
            #### OBS SPACE OF SIZE 12 + 1 
            #### Observation vector ### X        Y        Z       Q1   Q2   Q3   Q4   R       P       Y       VX       VY       VZ       WX       WY       WZ       Depth
            lo = -np.inf
            hi = np.inf
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            for i in range(self.NUM_DRONES):
                ...
                if self.step_counter % self.IMG_CAPTURE_FREQ == 0:
                    ...
                    depth_value = np.mean(dep) if dep is not None and dep.size > 0 else 0.0
                else:
                    
                    depth_value = getattr(self, 'last_depth', [0.0] * self.NUM_DRONES)[i]
                
                if not hasattr(self, 'last_depth'):
                    self.last_depth = [0.0] * self.NUM_DRONES
                self.last_depth[i] = depth_value
                
                obs_13[i, :] = np.hstack([obs[0:3], obs[7:10], obs[10:13], obs[13:16], depth_value]).reshape(13,)
