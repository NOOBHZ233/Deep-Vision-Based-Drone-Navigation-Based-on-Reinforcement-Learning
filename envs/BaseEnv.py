...
class BaseEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if self.VISION_ATTR:
            ...
            ###Drone-Camera###Drone-Camera###Drone-Camera###Drone-Camera###Drone-Camera###Drone-Camera
            self.CAM_WIDTH = 320
            self.CAM_HEIGHT = 240

            self.CAM_REL_POS = [0, 0, 0.05]  
            self.CAM_REL_ORN = p.getQuaternionFromEuler([0, 0, 0]) 
            ...
            self.CAM_PROJ = p.computeProjectionMatrixFOV(
                fov=self.CAM_FOV,
    ...
    def _getDroneImages(self,
        ...
        """
        Returns RGB, depth and segmentation images from the n-th drone POV.
        Depth is returned in real-world meters.

        Parameters
        ----------
        nth_drone : int
            Index of drone in self.DRONE_IDS.
        segmentation : bool
            Whether to generate segmentation mask.

        Returns
        -------
        rgb : ndarray  (H, W, 4)  uint8
        depth : ndarray (H, W)    float32  real depth in meters
        seg : ndarray   (H, W)    int
        """

        if self.IMG_RES is None:
            print("[ERROR] BaseAviary._getDroneImages(): self.IMG_RES is None.")
            exit()
        ...
        # ----------------------------------------
        cam_offset = np.array([0.05, 0.0, 0.05])
        cam_pos = pos + rot_mat.dot(cam_offset)
        ...
        view = p.computeViewMatrix(cameraEyePosition=cam_pos,
                                ...
                                cameraUpVector=up)
        proj = p.computeProjectionMatrixFOV(fov=60.0,
        ...
        SEG_FLAG = (
            ...
            if segmentation else p.ER_NO_SEGMENTATION_MASK
        )


        w, h, rgb, depth_buf, seg = p.getCameraImage(
    ...
    def _show_camera_view(self):
        ...
        rgb_display = rgb[:, :, [2, 1, 0]]

        
        dep_norm = (dep - dep.min()) / (dep.max() - dep.min() + 1e-6)
        dep_display = (dep_norm * 255).astype(np.uint8)
        dep_display_3ch = cv2.cvtColor(dep_display, cv2.COLOR_GRAY2BGR)

        rgbd_vis = np.hstack([rgb_display, dep_display_3ch])

        cv2.imshow("Drone Camera", rgbd_vis)
        cv2.waitKey(1)